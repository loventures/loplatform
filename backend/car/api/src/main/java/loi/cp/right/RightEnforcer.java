...
public class RightEnforcer implements AccessEnforcer {

    @Inject
    private RightService _rightService;

    @Infer
    private EnforceRight _enforce;

    @Override
    public boolean checkAccess() {
        return _rightService.getUserHasAtLeastOneRight(new HashSet<>(Arrays.asList(_enforce.value())));
    }
}
