...
import loi.cp.reverseproxy.ReverseProxyRoot

/** A shared startup task responsible for creating domain folders expected by admin machinery.
  *
  * @param domain
...
class CarAdminStartupTask(implicit
  domain: DomainDTO,
  fs: FacadeService
) extends StartupTask
    with FolderEnsurers:
  override def run(): Unit =
    ensureFolderByType(ReverseProxyRoot.ReverseProxyFolderType)
