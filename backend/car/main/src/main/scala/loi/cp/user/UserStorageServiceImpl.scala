...
@Service
class UserStorageServiceImpl(userStorageDao: UserStorageDao) extends UserStorageService:

  override def get[T: Storeable](user: UserDTO): T =
    val data = userStorageDao.loadUserStorage(user.id).map(_.getStoragedData).getOrElse(Json.jEmptyObject)
    decodeField(data)

  override def modify[T: Storeable](user: UserDTO)(f: T => T): T =
    val facade = userStorageDao.loadUserStorageForUpdate(user.id) match
