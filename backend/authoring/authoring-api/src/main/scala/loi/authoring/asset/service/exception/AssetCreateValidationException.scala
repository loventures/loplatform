...
case class AssetCreateValidationException(validationError: String, opIndex: Option[Int])
    extends UncheckedMessageException(
      BundleMessage(
        "asset.create.invalid",
        validationError,
        Int.box(opIndex.getOrElse(-1))
      )(using AuthoringBundle.bundle)
    )

object AssetCreateValidationException:

  def asTryFromMsg(msg: Option[String], opIndex: Option[Int]): Try[Unit] =
    msg.map(msg => AssetCreateValidationException(msg, opIndex)) match
