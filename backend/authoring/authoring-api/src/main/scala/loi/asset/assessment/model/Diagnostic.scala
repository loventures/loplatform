...
final case class Diagnostic(
  ...
  iconCls: String = "icon-EKG-line",
  @JsonDeserialize(contentAs = classOf[java.lang.Long])
  @Min(value = 1L)
  maxAttempts: Option[Long] = Some(1),
  @JsonDeserialize(contentAs = classOf[java.lang.Long])
  @Min(value = 1L)
  ...
  shouldDisplayConfidenceIndicator: Option[Boolean] = Some(true), // Here was false, frontend true
  ...
  scoringOption: Option[ScoringOption] = Some(ScoringOption.FirstAttemptScore),
...
object Diagnostic:
  ...
  implicit val assetTypeForDiagnostic: AssetType[Diagnostic] = new AssetType[Diagnostic](AssetTypeId.Diagnostic):
    ...
    override val edgeConfig: Map[Group, Set[AssetTypeId]] = Map(
      ...
      Group.TestsOut          -> AssetTypeId.GatedTypes,
    )

    override def validate(data: Diagnostic): ValidatedNel[String, Unit] =
    ...
    override def edgeIds(data: Diagnostic): Set[UUID] = Option(data.instructions).map(_.edgeIds).getOrElse(Set.empty)

    override def render(data: Diagnostic, targets: Map[UUID, Asset[?]]): Diagnostic =
  ...
  object Asset extends AssetExtractor[Diagnostic]
end Diagnostic
