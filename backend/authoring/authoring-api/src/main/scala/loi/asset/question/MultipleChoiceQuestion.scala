...
final case class MultipleChoiceQuestion(
...
object MultipleChoiceQuestion:
  ...
  implicit val assetTypeForMultipleChoiceQuestion: AssetType[MultipleChoiceQuestion] =
    new AssetType[MultipleChoiceQuestion](AssetTypeId.MultipleChoiceQuestion):
      ...
      override def validate(data: MultipleChoiceQuestion): ValidatedNel[String, Unit] =
      ...
      override def computeTitle(data: MultipleChoiceQuestion): Option[String] = Some(
        Option(data.questionContent.questionComplexText).map(_.plainText).getOrElse("")
      )

      override def receiveTitle(data: MultipleChoiceQuestion, title: String): MultipleChoiceQuestion =
        data.copy(questionContent = data.questionContent.receiveTitle(title))

      override def edgeIds(data: MultipleChoiceQuestion): Set[UUID] = data.questionContent.edgeIds

      override def render(data: MultipleChoiceQuestion, targets: Map[UUID, Asset[?]]): MultipleChoiceQuestion =
  ...
  object Asset extends AssetExtractor[MultipleChoiceQuestion]
