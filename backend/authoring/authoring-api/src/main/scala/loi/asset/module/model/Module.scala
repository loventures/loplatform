...
package loi.asset.module.model
...
object Module:
  ...
  implicit val assetTypeForModule: AssetType[Module] = new AssetType[Module](AssetTypeId.Module):

    override val edgeConfig: Map[Group, Set[AssetTypeId]] = Map(
      Group.Elements  -> AssetTypeId.LessonElementTypes.incl(AssetTypeId.Lesson),
      Group.Resources -> AssetTypeId.FileTypes,
    )

    override def validate(data: Module): ValidatedNel[String, Unit] =
      Validate.size("title", 1, 255)(data.title) *> Validate.size("keywords", max = 255)(data.keywords)

    // description editor is plaintext, some ancient modules have html tags in description.. import?
    override def index(
      data: Module
    ...
    override def htmls(data: Module)(implicit blobService: BlobService, mimeWebService: MimeWebService): List[String] =
      Nil

  object Asset extends AssetExtractor[Module]
end Module
