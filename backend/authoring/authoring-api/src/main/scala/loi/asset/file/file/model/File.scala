...
package loi.asset.file.file.model
...
object File:
  ...
  implicit val assetTypeForFile: AssetType[File] =
    new AssetType[File](AssetTypeId.File):
      override val allowedAttachmentTypes: Set[MediaType] = Set(MediaType.ALL)

      override def validate(data: File): ValidatedNel[String, Unit] =
      ...
      override def htmls(data: File)(implicit blobService: BlobService, mimeWebService: MimeWebService): List[String] =
        data.source.htmls

  object Asset extends AssetExtractor[File]
end File
