...
final case class HotspotQuestion(
  @NotEmpty
  title: String,
  @Size(min = 0, max = 255)
  keywords: String = "",
...
object HotspotQuestion:

  implicit val assetTypeForHotspotQuestion: AssetType[HotspotQuestion] =
    new AssetType[HotspotQuestion](AssetTypeId.HotspotQuestion):
      ...
      override def validate(data: HotspotQuestion): ValidatedNel[String, Unit] =
        Validate.notEmpty("title")(data.title) *>
          Validate.size("keywords", max = 255)(data.keywords)

      override def computeTitle(data: HotspotQuestion): Option[String] = Some(
        Option(data.questionContent.questionComplexText).map(_.plainText).getOrElse("")
      )

      override def receiveTitle(data: HotspotQuestion, title: String): HotspotQuestion =
        data.copy(questionContent = data.questionContent.receiveTitle(title))

      override def edgeIds(data: HotspotQuestion): Set[UUID] = data.questionContent.edgeIds

      override def render(data: HotspotQuestion, targets: Map[UUID, Asset[?]]): HotspotQuestion =
  ...
  object Asset extends AssetExtractor[HotspotQuestion]
end HotspotQuestion
...
case class HotspotQuestionContent(
  ...
  questionComplexText: HtmlPart = HtmlPart(renderedHtml = Some("")), // renderedHtml, html default is "asset.title"
  ...
  questionContentBlockText: BlockPart = BlockPart(
    parts = Seq(HtmlPart(renderedHtml = Some(""))),
    renderedHtml = Some("")
  ),                                                                 // html, renderedHtml default is "asset.title"
  ...
  hotspotVisible: Option[Boolean] = Some(true),
  choices: Seq[HotspotChoice] = Seq.empty
) extends QuestionContent:

  override def edgeIds: Set[UUID] =
    super.edgeIds

  def render(targets: Map[UUID, Asset[?]]): HotspotQuestionContent =
    copy(
  ...
  def receiveTitle(title: String): HotspotQuestionContent =
    copy(
...
end HotspotQuestionContent

case class HotspotChoice(
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_HOTSPOT_QUESTION_CHOICE_X_COORD) x: Double,
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_HOTSPOT_QUESTION_CHOICE_Y_COORD) y: Double,
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_HOTSPOT_QUESTION_CHOICE_RADIUS) radius: Double,
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_HOTSPOT_QUESTION_CHOICE_WIDTH) width: Double,
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_HOTSPOT_QUESTION_CHOICE_HEIGHT) height: Double,
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_HOTSPOT_QUESTION_CHOICE_SHAPE) shape: String,
  @JsonProperty(AssessmentQuestionTypeConstants.DATA_TYPE_ASSESSMENT_QUESTION_CHOICE_CORRECT) correct: Boolean
)
