...
final case class WebDependency(
  @Size(min = 1, max = 255)
  title: String,
  @Size(min = 0, max = 255)
  subtitle: String = "",
...
object WebDependency:

  implicit val assetTypeForWebDependency: AssetType[WebDependency] =
    new AssetType[WebDependency](AssetTypeId.WebDependency):

      override val edgeConfig: Map[Group, Set[AssetTypeId]] = Map(
        Group.Scripts     -> Set(AssetTypeId.Javascript),
        Group.Stylesheets -> Set(AssetTypeId.Stylesheet)
      )

      override def validate(data: WebDependency): ValidatedNel[String, Unit] =
  ...
  object Asset extends AssetExtractor[WebDependency]
end WebDependency
