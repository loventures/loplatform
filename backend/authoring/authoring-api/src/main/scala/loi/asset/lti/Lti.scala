...
object Lti:
  ...
  implicit val assetTypeForLti: AssetType[Lti] = new AssetType[Lti](AssetTypeId.Lti):
    ...
    override def validate(data: Lti): ValidatedNel[String, Unit] =
    ...
    override def edgeIds(data: Lti): Set[UUID] = Option(data.instructions).map(_.edgeIds).getOrElse(Set.empty)

    override def render(data: Lti, targets: Map[UUID, Asset[?]]): Lti =
    ...
    override def htmls(data: Lti)(implicit blobService: BlobService, mimeWebService: MimeWebService): List[String] =
      data.instructions.htmls

  object Asset extends AssetExtractor[Lti]
end Lti
