...
final case class Level2Competency(
  @Size(min = 1, max = 255)
  title: String,
  description: String = "",
  @Size(min = 0, max = 255)
...
object Level2Competency:

  implicit val assetTypeForLevel2Competency: AssetType[Level2Competency] =
    new AssetType[Level2Competency](AssetTypeId.Level2Competency):

      override val edgeConfig: Map[Group, Set[AssetTypeId]] = Map(
        Group.Level3Competencies -> Set(AssetTypeId.Level3Competency)
      )

      override def validate(data: Level2Competency): ValidatedNel[String, Unit] =
  ...
  object Asset extends AssetExtractor[Level2Competency]
end Level2Competency
