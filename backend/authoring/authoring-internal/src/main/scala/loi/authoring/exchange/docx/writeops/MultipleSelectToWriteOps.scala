...
trait MultipleSelectToWriteOps extends TableToWriteOps:
  ...
  override def writes(table: Table, competencies: Map[String, UUID]): ValNel[Warned[List[WriteOp]]] =
    ...
    for
      ...
      _          <- countValNel(cs, _ > 0)(_.isCorrect, RequiresSomeCorrectError(data.number))
      _          <- countValNel(cs, _ > 0)(_.isIncorrect, RequiresSomeIncorrectError(data.number))
      (w0, _)     = validateTupleRows(table)
      (w1, comps) = validateCompetencies(data, competencies)
    yield
      val correctCount   = cs.count(_.isCorrect)
      val incorrectCount = cs.size - correctCount
      val node           = AddNode(
          ...
          questionContent = ChoiceQuestionContent(
            ...
            choices = cs.zipWithIndex.map { case (c, i) =>
              ChoiceContent(
                index = i,
                correct = c.isCorrect,
                points = if c.isCorrect then 1.0 / correctCount else -1.0 / incorrectCount,
  ...
  private case class Choice(html: HtmlPart, isCorrect: Boolean, feedback: Option[HtmlPart]):
    def isIncorrect = !isCorrect
end MultipleSelectToWriteOps
