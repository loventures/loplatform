...
object Qti2ImageImporter:
  ...
  def addImages(request: QuestionImportRequest): (Node, Seq[NodeExchangeData], Map[String, UUID]) =
    ...
    /* ImageRewriteRule defined here so it can use the above mutable data structures. */
    ...
    val result: Node = ImageRewriter(node)
    (result, assets.toSeq, imageToEdgeMap.toMap)
  end addImages
end Qti2ImageImporter
