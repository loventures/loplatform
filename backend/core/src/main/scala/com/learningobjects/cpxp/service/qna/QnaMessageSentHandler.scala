...
class QnaMessageSentHandler extends QueryHandler:
  override def applyFilter(qb: QueryBuilder, filter: ApiFilter): Unit =
    if filter.getOperator != null then filter.unsupported()
    val comparison = if filter.getValue == "true" then Comparison.ne else Comparison.eq
    qb.addCondition(BaseCondition.getInstance(QnaQuestionFinder.Recipients, comparison, null))
