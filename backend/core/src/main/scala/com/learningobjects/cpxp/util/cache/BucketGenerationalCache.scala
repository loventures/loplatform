...
// TODO: timeout from detomcat conf
...
/** @param timeout
  ...
  *   a source for time
  */
class BucketGenerationalCache[K <: Serializable, V >: Null, E <: Entry[K, V]](
  val itemAware: Boolean,
  val replicated: Boolean,
  timeout: FiniteDuration
) extends Cache[K, V, E]:
  import BucketGenerationalCache.*

  private final val ts = TimeSource.realtime

  private final val logger = org.log4s.getLogger(getClass)
  ...
  override def resetStatistics(): Unit = synchronized {
    hitCount = 0
    missCount = 0
  }

  override def getSize: Int = synchronized {
    buckets.map(_.size).sum
  }

  override def getInvalidationSize: (Int, Int) = synchronized {
    buckets.map(_.invalidationSize).suml
  }

  override def put(entry: E): Unit = synchronized {
  ...
  override def test(key: K): Boolean = synchronized {
    validBuckets exists { b =>
      b.contains(key)
    }
  }

  override def getKeys: List[K] = synchronized {
  ...
  override def remove(key: K, propagate: Boolean): Unit = synchronized {
    buckets
...
end BucketGenerationalCache

object BucketGenerationalCache:
