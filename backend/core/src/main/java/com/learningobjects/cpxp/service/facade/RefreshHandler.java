...
/**
 * Handler for a facade method that refreshs an item.
 */
class RefreshHandler implements FacadeMethodHandler {
    @Override
    public Object invoke(FacadeInvocationHandler handler, Object[] args) {
        ...
        if ((args != null) && (args.length == 1) && (args[0] != null)) {
            ...
            return handler.getContext().getItemService().lock(item, lock, true, timeout);
        } else {
            return handler.getContext().getItemService().lock(item, false, true, Optional.empty());
        }
    }
