...
final class SubmissionAssessmentProgressEventHandler(
  ...
  override def onEvent(params: SubmissionAttemptActionParameters, event: SubmissionAttemptEvent): Try[Unit] =
    event match
      ...
      case AttemptInvalidatedEvent if noMoreAttempts(params) => updateProgress(params, List(ProgressChange.unvisit))
      case _                                                 => Success(())

  private def noMoreAttempts(params: SubmissionAttemptActionParameters): Boolean =
    !submissionAttemptLoadService
      .getUserAttempts(params.assessment.section, Seq(params.assessment), params.attempt.user)
      .exists(_.valid)

  private def updateProgress(
    params: SubmissionAttemptActionParameters,
    changes: List[EdgePath => ProgressChange]
  ): Try[Unit] =
    val gradebook = gradeService.getGradebook(params.assessment.section, params.attempt.user)
    progressService
      .updateProgress(
        params.assessment.section,
        params.attempt.user,
        gradebook,
        changes.map(_.apply(params.assessment.edgePath))
      )
      .toTry(e => new RuntimeException(e.msg))
      .map(_ => ())
  end updateProgress
end SubmissionAssessmentProgressEventHandler
