...
import loi.cp.submissionassessment.attempt.event.{AttemptSubmittedEvent, SubmissionAttemptEvent}
import loi.cp.submissionassessment.attempt.{IllegalResponseState, SubmissionAttempt, SubmissionAttemptFailure}
import scalaz.\/
import scalaz.syntax.either.*

/** An action to submit the response for a [[SubmissionAttempt]]. This marks the end of the subject's input and allows
  * for grading.
  */
case class SubmitAttempt() extends SubmissionAttemptAction:
  ...
  override def exec(parameters: SubmissionAttemptActionParameters): SubmissionAttemptFailure \/ SubmissionAttempt =
    if parameters.attempt.state == AttemptState.Open then
      parameters.attempt.copy(state = AttemptState.Submitted, submitTime = Some(parameters.time)).right
    else IllegalResponseState("Cannot submit to a non-open attempt").left

  override def events(parameters: SubmissionAttemptActionParameters): Seq[SubmissionAttemptEvent] =
    Seq(AttemptSubmittedEvent(false))
