...
public interface GroupFolderFacade extends FolderFacade, ComponentFacade {
    public void lock(boolean pessimistic);

    @FacadeChild(GroupConstants.ITEM_TYPE_GROUP)
    public List<GroupFacade> getGroups();
    public GroupFacade getGroup(final Long id);
    public GroupFacade findGroupByGroupId(
      @FacadeCondition(value = GroupConstants.DATA_TYPE_GROUP_ID, function = Function.LOWER)
      String groupId
    );
    @FacadeQuery(cache = false)
    public List<GroupFacade> findGroupsBySubquery(
      @FacadeCondition(value = DataTypes.META_DATA_TYPE_ITEM, comparison = Comparison.in)
      QueryBuilder subQuery
    );
    public QueryBuilder queryGroups();
