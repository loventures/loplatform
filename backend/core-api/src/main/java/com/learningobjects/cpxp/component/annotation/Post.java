...
/**
 * Exposes a function to be accessible via an RPC call as a POST request
 ...
 * For example, to invoke 'deleteEnrollment' on EnrollmentView.java:
 *     _rpcBase = "$$rpc('')"
 *     ...
 *     ug.rpc(_rpcBase + 'deleteEnrollment', { enrollment: selectedRow.id }, <callback>);
 * Note that the $$rpc(...) class is implied by context, because enrollmentView.html was invoked from the 'render' call in EnrollmentView.java
 */
@Retention(RetentionPolicy.RUNTIME)
@Target(ElementType.METHOD)
@Function
@RpcMethod(Method.POST)
