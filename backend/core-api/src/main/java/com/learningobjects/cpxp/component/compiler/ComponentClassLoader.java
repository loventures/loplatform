/*
 * LO Platform copyright (C) 2007–2025 LO Ventures LLC.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

package com.learningobjects.cpxp.component.compiler;

import javax.tools.JavaFileObject;
import java.io.InputStream;
import java.net.URL;
import java.util.SortedMap;

public interface ComponentClassLoader {
    boolean containsClass(String name);

    SortedMap<String, JavaFileObject> getClassFiles();

    Package findPackage(String name);

    URL findResource(String name);

    Class<?> getClass(String name);

    /**
     * Add a class name/JavaFileObject mapping
     *
     * @param qualifiedClassName
     *           the name
     * @param javaFile
     *           the file associated with the name
     */
    void addClass(String qualifiedClassName, JavaFileObject javaFile);

    void addResource(String qualifiedClassName, URL url);

    InputStream getResourceAsStream(String name);

    Class<?> loadClass(String name) throws ClassNotFoundException;
}
