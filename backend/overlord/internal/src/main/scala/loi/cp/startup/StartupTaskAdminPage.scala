...
import loi.cp.overlord.OverlordSecurityGuard

@Component
@AdminPageBinding(
  group = "Overlord",
  secured = new Secured(value = Array(classOf[HostingAdminRight]), guard = Array(classOf[OverlordSecurityGuard]))
)
class StartupTaskAdminPage(
  val componentInstance: ComponentInstance
) extends AdminPageComponent
    with ComponentImplementation:
  override def renderAdminPage(): WebResponse =
    HtmlResponse(this, "startupTaskAdminPage.html")
end StartupTaskAdminPage
