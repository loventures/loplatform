...
export function getMd5Hash(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    new BMF().md5(file, function (err: Error | null, md5: string) {
      resolve(md5);
      reject(err);
    });
  });
...
export function md5HashToObjectKey(domainId: string | number, md5: string): string {
  return `authoring/${domainId}/${md5[0]}/${md5[1]}/${md5[2]}/${md5.slice(3)}`;
}

export function getMimeFromExtension(file: { name: string }): string {
