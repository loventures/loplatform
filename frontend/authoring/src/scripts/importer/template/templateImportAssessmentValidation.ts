...
function validateScoringOption(scoringOptionFromInput, { maxAttempts }: any, typeId) {
...
function validateConfidenceIndicators(displayConfidenceIndicatorsFromInput, { singlePage }: any) {
...
function validateImmediateFeedback(immediateFeedbackFromInput, { singlePage }: any) {
...
function validateIndividualSettings(settings, typeId): any {
...
function validateAssessmentStructure(row: Record<string, any>, typeId: string): any {
...
export function validateAssessmentSettings(row: Record<string, any>, typeId: string): any {
  ...
  const errorCollection = Object.values(maybeSettings).filter((v: any) => v.err);
