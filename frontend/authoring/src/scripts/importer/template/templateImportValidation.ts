...
export function generateWriteOps(
  file: File,
...
function validateTitle(titleFields: any[]): any {
...
function validateTypeId(rawTypeId: string = ''): any {
  if (rawTypeId === '') {
...
function validatePath(currentPath: any[], currentLevel: number, newNodeName: string): any {
...
function validateSourceNode(currentLevel: number, currentAssetType: any, sourceNode: any): any {
  if (
...
function validateStructure(currentLevel: number, typeId: string): any {
...
function validateLvl1(typeId: string): any {
...
function validateLvl2(typeId: string): any {
...
function validateLvl3(typeId: string): any {
...
function validateCompetencies(
  ...
  competenciesByTitle: Record<string, any>
): any {
...
function validateCategory(
  ...
  categoriesByTitle: Record<string, any>
): any {
  if (!title?.trim()) {
    return '';
...
export function validateRows(
  ...
  competencies?: any[],
  categories?: any[]
): { ops: any[]; errors: any[]; warnings: any[] } {
  const competenciesByTitle: Record<string, any> = {};
  for (const competency of competencies ?? []) {
    competenciesByTitle[competency.data.title.replaceAll(NonAlpha, '').toLowerCase()] = competency;
  }
  const categoriesByTitle: Record<string, any> = {};
  ...
  const { ops, errors, warnings } = rows.reduce(
    (acc: any, row, idx) => {
      ...
      if (
        ...
        const orderedSiblingEdgeNames = Object.values<any>({ ...acc.edges, ...edges })
...
function cleanOps(ops: any[]): any[] {
  const compressedSeos = ops.reduce((acc: any[], op) => {
...
function cleanErrors(errors: any[]): any[] {
  return errors
    .reduce((acc: any[], error, idx) => {
