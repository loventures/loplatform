...
const AssessmentImportModal = ({
  ...
  assessmentNode,
  dispatch,
}: AssessmentImportModalProps) => {
  const validFileTypes = [...importExtensions.qti, ...importExtensions.loqi];
  const [file, setFile] = useState<File | undefined>(dropzoneFile);
  ...
  const [{ errors, preview, warnings }, setModalData] = useState<ModalData>({
    errors: [],
    preview: '',
    warnings: [],
  });
  const [loading, setLoading] = useState(false);
  const [qtiImport, setQtiImport] = useState<any>({});
  const [loqiImport, setLoqiImport] = useState<any>({});
...
const mapStateToProps = (state: any) => {
  const { branchId } = reactRouterService.getCurrentParams<{ branchId?: string }>(state);
...
export default withTranslations(connect(mapStateToProps)(AssessmentImportModal as any)) as React.ComponentType<any>;
