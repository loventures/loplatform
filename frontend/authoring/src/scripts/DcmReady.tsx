...
import { useRouterPathVariable } from './hooks/routes';
...
import { fetchAnnouncements } from './announcement/AnnouncementActions';

type LoPlatform = Window['lo_platform'];

interface DcmReadyProps {
  loPlatform: LoPlatform;
  ready: boolean;
  dispatch: Dispatch;
  match: { params: { branchId?: string } };
}

interface DcmReadyState {
  ready: boolean;
  domain: any;
  authoring: any;
  translations: any;
  loggedOut: boolean;
  sessionInterval?: ReturnType<typeof setInterval>;
}

class DcmReady extends Component<DcmReadyProps, DcmReadyState> {
  constructor(props: DcmReadyProps) {
    super(props);
  ...
  componentDidUpdate(prevProps: DcmReadyProps) {
...
const ConnectedDcmReady = connect(mapStateToProps)(DcmReady as any) as React.ComponentType<any>;

// v6 removed withRouter (and class components can't use hooks). DcmReady only needs the URL's
// branchId, so inject a synthetic `match` from the router-derived path variable; it recomputes on
// navigation, so the class's componentDidUpdate(branchId change) re-bootstrap still fires.
const DcmReady_ = (props: { loPlatform: LoPlatform }) => {
  const branchId = useRouterPathVariable('branchId');
  return (
    <ConnectedDcmReady
      {...props}
...
export default DcmReady_;

/* Private translation helper */

export const getLocalI18nData = (serverI18n: any, locale: string) => {
  if (process.env.NODE_ENV === 'development') {
    return  import(/* @vite-ignore */`../i18n/Authoring_${locale.substring(0, 2)}.csv`)
      .then((module) =>
        module.default.map(({key, value}: { key: string; value: string }) => [key, value]))
      .then(rows => {
        const localI18n: Record<string, string> = fromPairs(rows);
        const expandedI18n = mapValues(localI18n, val => {
          return val.replace(/``([^`]*)``/g, (m: string, g: string) => localI18n[g]).replace(/""/g, '"');
        });
