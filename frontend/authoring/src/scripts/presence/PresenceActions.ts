...
export const setPresenceState = (state: unknown) => ({
  type: SET_PRESENCE_STATE,
  state,
});

export const setTabVisible = (visible: boolean) => ({
  type: SET_TAB_VISIBLE,
  visible,
});

export const setIdleState = (idling: boolean) => ({
  type: SET_IDLE_STATE,
  idling,
});

export const setPresenceId = (presenceId: string | null) => ({
  type: SET_PRESENCE_ID,
  presenceId,
});

export const setProfiles = (profiles: unknown) => ({
  type: SET_PROFILES,
  profiles,
});

export const branchReindexing =
  (branch: number, reindexing: boolean) => (dispatch: any) => {
...
export const fetchProfiles = (handles: string[]) => (dispatch: Dispatch, getState: any) => {
  const {
    presence: { profiles },
  } = getState();
...
export const setPresentUsers =
  (users: Array<[string, number, string | null]>) => (dispatch: any, getState) => {
...
export const historyLoaded = (room: string, messages: unknown[]) => ({
  type: HISTORY_LOADED,
  room,
  messages,
});

export const loadHistory = (room: string) => (dispatch: any, getState: any) => {
  const {
...
export const openChat = (branch: number, room: string) => (dispatch: any) => {
  dispatch({
    type: OPEN_CHAT,
    branch,
    room,
...
export const toggleBranchChat = (branch: any) => (dispatch: any, getState: any) => {
  const {
...
export const sendTyping = (room: string, typing: boolean) => () => {
