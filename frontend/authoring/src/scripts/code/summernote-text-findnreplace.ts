...
// Taken from https://github.com/DiemenDesign/summernote-text-findnreplace.git#8614858
// and fixed to actually work wtf
...
$.extend($.summernote.plugins, {
  findnreplace: function (this: any, context: any) {
    ...
    this.initialize = function (this: any) {
      var fnrBody =
        '<div id="findnreplaceToolbar" class="note-display-none">' +
        '<div class="note-form-group">' +
        '<input id="note-findnreplace-find" type="text" class="note-findnreplace-find note-input" value="" placeholder="' +
        lang.findnreplace.findPlaceholder +
        '">' +
        '<button class="note-findnreplace-find-btn note-btn">' +
        lang.findnreplace.findBtn +
        '</button>' +
        '</div>' +
        '<div class="note-form-group">' +
        '<input id="note-findnreplace-replace" type="text" class="note-findnreplace-replace note-input" value="" placeholder="' +
        lang.findnreplace.replacePlaceholder +
        '">' +
        '<button class="note-findnreplace-replace-btn btn btn-default note-btn" style="width:100px;">' +
        lang.findnreplace.replaceBtn +
        '</button>' +
        '</div>' +
        '</div>';
    ...
    this.findnreplace = function () {
      ...
      $fnrFindBtn.click(function (e) {
        ...
        if (fnrFind) {
          $('.note-status-output').text(
            fnrCount + lang.findnreplace.findResult + '`' + fnrFind + '`'
          );
          var fnrReplaced = fnrCode.replace(
            new RegExp(fnrFind + '(?![^<>]*>)', 'gi'),
            function (e) {
              return '<mark class="note-findnreplace">' + e + '</mark>';
            }
          );
          $note.summernote('code', fnrReplaced);
        } else $('.note-status-output').html(lang.findnreplace.findError);
      });
      $fnrReplaceBtn.click(function (e) {
        ...
        if (fnrFind) {
          $('.note-status-output').text(
            fnrCount +
              lang.findnreplace.findResult +
              '`' +
              fnrFind +
              '`' +
              lang.findnreplace.replaceResult +
              '`' +
              fnrReplace +
              '`'
          );
          ...
          if (fnrReplace) {
            if ($note.summernote('createRange').toString()) {
              ...
              $('.note-status-output').text('');
            } else $('.note-status-output').html(lang.findnreplace.noneSelected);
          } else $('.note-status-output').html(lang.findnreplace.replaceError);
        }
      });
    };
