...
const normaliseWs = (cleanCode: string): string => {
...
const beautifyCodeImpl = (dirtyCode: string, dataIds?: boolean): string => {
  let cleanCode = dirtyCode;
  const injectDataIds =
    (window.lo_platform as any)?.config.effective.injectDataIds && dataIds !== false;
  ...
  if (injectDataIds && !cleanCode.includes('data-id="')) {
    ...
    soapBox.find(idTags).each(function (this: HTMLElement) {
  ...
  soapBox[0].innerHTML = cleanCode;
  soapBox.find('p.ul-li').each(function (this: HTMLElement) {
    $(this).wrap('<li class="ul-li" />');
  });
  soapBox.find('p.ol-li').each(function (this: HTMLElement) {
    $(this).wrap('<li class="ol-li" />');
  });
  soapBox.find('p.ol-li, p.ul-li').contents().unwrap();
  soapBox.find('li.ol-li').each(function (this: HTMLElement) {
    if (!$(this).prev().is('li.ol-li')) {
      $(this).addClass('first-li');
    }
  });
  soapBox.find('li.ul-li').each(function (this: HTMLElement) {
    if (!$(this).prev().is('li.ul-li')) {
      $(this).addClass('first-li');
    }
  });
  soapBox.find('li.ol-li.first-li').each(function (this: HTMLElement) {
    $(this).nextAll('li.ol-li').andSelf().wrapAll('<ol></ol>');
  });
  soapBox.find('li.ul-li.first-li').each(function (this: HTMLElement) {
    $(this).nextAll('li.ul-li').andSelf().wrapAll('<ul></ul>');
  });
  soapBox.find('li.ol-li, li.ul-li').each(function (this: HTMLElement) {
    $(this).removeAttr('class');
  });
  soapBox.find('img').each(function (this: HTMLElement) {
...
export const beautifyCode = (dirtyCode: string, dataIds?: boolean): string => {
  try {
