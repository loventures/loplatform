...
$.extend($.summernote.plugins, {
  cleaner: function (this: any, context: any) {
    ...
    this.events = {
      ...
      'summernote.paste': function (we, event) {
        if (options.cleaner.action == 'both' || options.cleaner.action == 'paste') {
          ...
          if (msie) text = (window as any).clipboardData.getData('Text');
          else {
            ...
            if ($editor.find('.note-status-output').length > 0) {
              ...
              setTimeout(function () {
                if ($editor.find('.note-status-output').html() == lang.cleaner.not) {
                  ...
                  $editor.find('.note-status-output').fadeOut(function (this: any) {
                    $(this).html('');
                    $(this).fadeIn();
                  });
                }
    ...
    var cleanHtmlPaste = function (
      ...
      // @ts-ignore -- LOV: original `i < badTags?.length ?? 0` precedence quirk, kept verbatim
      ...
      // @ts-ignore -- LOV: original `j < keepTagContents?.length ?? 0` precedence quirk, kept verbatim
      ...
      // @ts-ignore -- LOV: original `k < badAttributes?.length ?? 0` precedence quirk, kept verbatim
