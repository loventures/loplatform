...
import { GradableEditor } from './components/GradableEditor';
import { PointsEditor } from './components/PointsEditor';

export const CourseLinkSettings: NarrativeSettings<'courseLink.1'> = ({ asset }) => {
  const polyglot = usePolyglot();
  const editMode = useIsEditable(asset.name, 'EditSettings');
  const { gradable, duration, isForCredit, pointsPossible } = asset.data;
  ...
  return editMode ? (
    ...
    <div className="mx-3 mb-2 d-flex justify-content-center">
      <span className="input-padding text-muted text-center feedback-context">
        {sentence(
          polyglot.t(!gradable ? 'STORY_ungraded' : `STORY_forCredit_${isForCredit}`),
          gradable && categoryTitle,
          gradable && plural(pointsPossible, 'point'),
