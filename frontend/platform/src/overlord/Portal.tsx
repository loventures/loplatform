...
import { ClusterHealthyUrl, StartupTasksStatusUrl } from '../services/URLs';
import allPages, { OverlordPage } from './pages';

interface CardProps {
  audio?: () => Promise<{ default: string }>;
  child?: { identifier: string; name?: string };
  className?: string;
  href?: string;
  identifier: string;
  icon?: string;
...
const Card: React.FC<CardProps> = ({
  ...
  const onMouseOver = () => {
    timeout.current = setTimeout(playSound, 7000);
  };
    ...
    >
      {!href ? (
        ...
        >
          ...
          <div className="portal-card-title">{label || T.t(`overlord.page.${identifier}.name`)}</div>
        </Link>
      ) : (
        <a
          className="portal-card-link"
...
const Portal: React.FC = () => {
  ...
  const { user } = lo_platform;

  const [pages, setPages] = useState<OverlordPage[]>(() =>
    allPages.filter(p => isDevelopment || hasRight(user, p.right))
  );
  const [clusterHealthy, setClusterHealthy] = useState(true);
  const [startupHealthy, setStartupHealthy] = useState(true);

  const onClusterHealth = ({ data: healthy }: { data: boolean }) => {
    setClusterHealthy(healthy);
    if (!healthy) {
      window.setTimeout(pollClusterStatus, ThreeDecaKiloMilliSeconds);
    }
  };

  const pollClusterStatus = () => axios.get(ClusterHealthyUrl).then(onClusterHealth);

  const onStartupTaskStatus = ({ data: status }: { data: string }) => {
    if (status !== 'Success') {
      setPages(prev =>
        prev.map(page => {
      ...
      setStartupHealthy(status !== 'Failure');
      if (status === 'InProgress') {
        window.setTimeout(pollStartupTaskStatus, ThreeDecaKiloMilliSeconds);
      }
    } else {
      setStartupHealthy(true);
    }
  };

  const pollStartupTaskStatus = () => axios.get(StartupTasksStatusUrl).then(onStartupTaskStatus);

  useEffect(() => {
    if (pages.find(p => p.identifier === StartupTasksIdentifier)) {
      pollStartupTaskStatus();
      pollClusterStatus();
    }
  ...
  const onClick = () => {
    setPages(prev => {
      const index = parseInt(String((1 + prev.length) * Math.random()), 10);
      const icon = iconia[parseInt(String(iconia.length * Math.random()), 10)];
      const label = icon
        .split('_')
        .map(a => a.substring(0, 1).toUpperCase() + a.substring(1))
        .join(' ');
      const portlet: OverlordPage = {
        identifier: 'xxx-' + prev.length,
        icon,
        label,
        href: '/',
        right: '',
      };
      return [...prev.slice(0, index), portlet, ...prev.slice(index)];
    });
  };
