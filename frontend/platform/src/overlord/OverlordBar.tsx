...
import { User } from '../types/loPlatform';
import { isDevelopment } from '../services';
import { hasRight, OverlordRight } from '../services/Rights';
...
const optedOutOfNiceThings = (user: User | null | undefined) =>
  user && optOutEes.indexOf(user.userName ?? '') >= 0;

interface OverlordBarProps {
  page?: string | null;
  simple?: boolean;
}

const OverlordBar: React.FC<OverlordBarProps> = ({ page, simple }) => {
  const lo_platform = useLoPlatform();

  const clusterName = (lo_platform.clusterName as string)
    .split('-')
    .map(s => s.substring(0, 1).toUpperCase() + s.substring(1))
    .join(' ');

  const divs = (i: number): React.ReactElement<any> => <div> {i > 0 ? divs(i - 1) : null} </div>;
