...
export default (
  T: Polyglot,
  updateDomainInfo: (info: Record<string, any>) => void
  ...
  const updateData = () => {
    ...
    Object.keys(domainInfo).forEach(field => {
      const value = domainInfo[field as keyof typeof domainInfo];
      if (value.length === 0) {
        errors.push(
    ...
    return errors.length === 0
      ? axios
          ...
          .catch(({ response: { data } }) => {
            return Promise.reject(data.messages.map((m: { message: string }) => m.message));
          })
