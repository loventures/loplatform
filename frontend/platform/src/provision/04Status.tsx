...
export default (
  ...
  const [evSrc, setEvSrc] = useState<EventSource | null>(null);
  ...
  const onMessage =
    ...
    ({ data }: MessageEvent) => {
      ...
      if (msg.status === 'warning') {
        const key = msg.body.message as keyof Status;
        if (!statusAcc[key]) {
          statusAcc[key] = true;
          setStatus(statusAcc);
        }
  ...
  const className = (prop: keyof Status) => {
    const activeClass = () =>
      (Object.keys(status) as (keyof Status)[]).filter(p => !status[p])[0] === prop
        ? 'active'
        : 'inactive';
  ...
  return (
    <Col className="provision-form">
      <div>
        <ol>
          {(Object.keys(status) as (keyof Status)[]).slice(0, 6).map(prop => (
            <li
              key={prop}
