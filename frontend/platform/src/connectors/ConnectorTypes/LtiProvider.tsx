...
const LtiProvider: React.FC<ConnectorComponentProps> = ({ T, row, configs, renderField }) => {
  ...
  return (
    <React.Fragment>
      {configs &&
        configs.filter(config => config.id !== 'configuration').map(config => renderField(config))}
      <AdminFormCheck
        entity="connectors"
