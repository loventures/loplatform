...
/**
 * Taken from https://github.com/carbonblack/react-console
 * Taken from https://github.com/snowak-cb/react-console
 * Patched by LoVe to work with our lesser typescript, have better style and fixed scrolling and keys.
    ...
    if (this.props.point < 0) {
      ...
      return [
        ...
        <span
          ...
          className="react-console-cursor"
        >
          {this.props.value.substring(this.props.point, this.props.point + 1)}
        </span>,
...
let ConsoleTableHeader: React.SFC<ConsoleTableHeaderProps> = function (
  props: ConsoleTableHeaderProps
) {
  if (props.headers) {
    return (
      <thead>
...
let ConsoleMessage: React.SFC<ConsoleMessageProps> = function (props: ConsoleMessageProps) {
  if (props.isTable) {
    ...
    return (
      <div
        className={
          'react-console-message react-console-table' +
          (props.type ? ' react-console-message-' + props.type : '')
        }
      >
        <table>
    ...
    return (
      <div
        className={
          'react-console-message' + (props.type ? ' react-console-message-' + props.type : '')
        }
      >
        {props.value.map((val: any, i: number) => {
          ...
          } else if (typeof val === 'object' && val.className) {
            return (
              <div
                key={i}
...
export default class extends React.Component<ConsoleProps, ConsoleState> {
  ...
  updateLastLog = (...messages: any[]) => {
    ...
    let indexToReplace =
      log[this.state.log.length - 1].message.length > 0
        ? log[this.state.log.length - 1].message.length - 1
        : 0;
  ...
  logTable = (tableData: ConsoleTableObject, type?: string) => {
    let log = this.getSafeLog();
    if (type) {
      log[this.state.log.length - 1].message.push({
        isTable: true,
        type: type,
        value: [tableData],
      });
    ...
    if (this.state.acceptInput) {
      if (e.altKey) {
        ...
        if (e.keyCode in ctrlCodes) {
          ...
          // allow ctrl+v for paste on windows
          e.preventDefault();
        }
  ...
  change = () => {
    ...
    if (this.state.lastCommand == ConsoleCommand.Search) {
      this.setState(
        {
          searchText: this.state.searchInit
            ? insert
            : this.textInsert(insert, this.state.searchText, replace),
      ...
      this.setState(
        Object.assign(this.consoleInsert(insert, replace), {
          typer: this.child.typer.value,
          lastCommand: ConsoleCommand.Default,
        }),
        this.scrollToBottom
  ...
  paste = (e: ClipboardEvent) => {
    ...
    if (this.state.lastCommand == ConsoleCommand.Search) {
      this.setState(
        {
          searchText: this.state.searchInit
            ? insert
            : this.textInsert(insert, this.state.searchText),
          typer: this.child.typer.value,
        },
        this.triggerSearch
      );
      ...
      this.setState(
        Object.assign(this.consoleInsert(insert), {
          lastCommand: ConsoleCommand.Default,
        }),
        this.scrollToBottom
      );
  ...
  acceptLine = () => {
    ...
    if (this.props.continue(this.state.promptText)) {
      this.setState(
        Object.assign(this.consoleInsert('\n'), {
          typer: '',
          lastCommand: ConsoleCommand.Default,
        }),
        this.scrollToBottom
  ...
  reverseSearchHistory = () => {
    if (this.state.lastCommand == ConsoleCommand.Search) {
      this.setState(
        Object.assign(this.searchHistory(SearchDirection.Reverse, true), {
  ...
  forwardSearchHistory = () => {
    if (this.state.lastCommand == ConsoleCommand.Search) {
      this.setState(
        Object.assign(this.searchHistory(SearchDirection.Forward, true), {
  ...
  deleteChar = () => {
    if (this.state.point < this.state.promptText.length) {
      this.setState(
        {
          promptText:
            this.state.promptText.substring(0, this.state.point) +
      ...
      this.setState(
        {
          point: this.movePoint(-1),
          promptText:
            this.state.promptText.substring(0, this.state.point - 1) +
    ...
    this.setState(
      {
        point: this.previousWord(),
        promptText:
          this.state.promptText.substring(0, this.previousWord()) +
          this.state.promptText.substring(this.state.point),
        kill: kill,
        killn: 0,
        argument: null,
  ...
  yank = () => {
    this.setState(
      Object.assign(this.consoleInsert(this.state.kill[this.state.killn]), {
  ...
  consoleInsert = (insert: string, replace: number = 0): ConsoleState => {
    ...
    return {
      point: this.movePoint(
        insert.length - replace,
        insert.length - replace + this.state.promptText.length
      ),
  ...
  searchHistory = (
    direction: SearchDirection = this.state.searchDirection,
    next: boolean = false
  ): ConsoleState => {
  ...
  scrollIfBottom = () => {
    if (
      ...
      this.child.container.scrollTop >=
        this.child.container.scrollHeight - this.child.container.offsetHeight
    ) {
  ...
  render() {
    return (
      ...
      >
        ...
        {this.state.log.map((val: LogEntry, jdx: number) => {
          return [
            <ConsolePrompt
              key={`prompt-${jdx}`}
              label={val.label}
