...
import { setLoPlatform, setPortalAlertStatus } from '../redux/actions/MainActions';
import { useTranslations } from '../redux/state';
import { DomainSettingsUrl, LocalesUrl, TimeZonesUrl } from '../services/URLs';
import { IoOptionsOutline } from 'react-icons/io5';
import { getPlatform } from '../services';

const requiredFields: Record<string, boolean> = { name: true, shortName: true };

const DomainSettings: React.FC & AdminPage = () => {
  ...
  useEffect(() => {
    axios
      .all([DomainSettingsUrl, TimeZonesUrl, LocalesUrl].map(url => axios.get(url)))
      .then(
        axios.spread((domainSettings, timeZones, locales) => {
          dispatch(setPortalAlertStatus(false, false, ''));
          setLoaded(true);
          setSettings(domainSettings.data);
          setTimeZones(
            timeZones.data.objects.map((o: any) => ({
              ...o,
              displayString: T.t('adminForm.format.nameId', o),
            }))
          );
          setLocales(
            locales.data.objects.map((o: any) => ({
              ...o,
              displayString: T.t('adminForm.format.nameId', o),
            }))
          );
  ...
  const clearPortalAlert = () => dispatch(setPortalAlertStatus(false, false, ''));

  const submitSettings = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    const el = e.target as HTMLFormElement;
    const form: any = serialize(el, { hash: true, empty: true });
    ...
    if (missing) {
      ...
      setValidationErrors({ [missing]: T.t('adminForm.validation.fieldIsRequired', params) });
      dispatch(setPortalAlertStatus(true, false, T.t('adminPage.domainSettings.alert.formError')));
    } else {
      setSubmitting(true);
      setValidationErrors({});
      clearPortalAlert();
        ...
        .catch(err => {
          ...
          if (data && data.type === 'VALIDATION_ERROR' && Array.isArray(data.messages)) {
            const validationErrors = data.messages.reduce(
              (result: Record<string, string>, { property, message }: any) => {
  ...
  const fieldUrl = (field: string) => `${DomainSettingsUrl}/${field}`;
