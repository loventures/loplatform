...
import { makeRecipient } from './makeRecipient.ts';
...
const SendMessage = ({ entireClass, recipients }: SendMessageProps) => (
  <FullMessageCreator
...
const selectSendMessage = createSelector(selectRouter, router => ({
  ...
  recipients: map(b64Decode(router.searchParams.recipients), l => makeRecipient(l)),
}));
