...
export const makePostStateUpdateActionCreator = (discussionId: any) => {
  const selectBoard = createBoardSelector(discussionId);

  const initState = function (post: any, board: any, currentUser: any) {
  ...
  return (posts: any) => (dispatch: any, getState: () => CourseState) => {
