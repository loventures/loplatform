...
import { createCourseSelector, selectCurrentUser, selectPreferences, selectCourse } from '../utilities/rootSelectors.js';
...
export const selectPosts = (state: CourseState) => state.api[postsDataSlice];

export const selectThreads = (state: CourseState) => state.api[threadsDataSlice];

export const selectPostsState = (state: CourseState) => state.ui[discussionsSlice][postsStateSlice];

export const selectThreadsState = (state: CourseState) => state.ui[discussionsSlice][threadsStateSlice];

export const selectBoardsState = (state: CourseState) => state.ui[discussionsSlice][boardsStateSlice];

export const selectViewsState = (state: CourseState) => state.ui[discussionsSlice][viewsSlice];

export const selectJumpersState = (state: CourseState) => state.ui[discussionsSlice][jumpersSlice];

export const selectSearchState = (state: CourseState) => state.ui[discussionsSlice][searchSlice];

export const selectRouterState = (state: CourseState) => state.router;

export const selectPostDetails = createCourseSelector(
  selectPosts, selectPostsState,
  (posts, postsState): Record<string, any> =>
...
export const selectPostDetailsWithUserInfo = createCourseSelector(
  selectPostDetails, selectCurrentUser, selectRouterState, selectCourse,
  (posts, currentUser, router, course): Record<string, any> =>
    mapValues(posts, post => {
      const isCurrentUserPost = post.author.handle === currentUser.handle;
      const itemClasses = processItemClasses(post);
      const stateParams = router.location.state; // this is wrong, always undefined
...
const selectPostDetailsWithUserInfoByThread = createCourseSelector(
  selectPostDetailsWithUserInfo,
  (posts): Record<string, any[]> => groupBy(posts, 'threadId')
);

export const selectThreadsWithChildren = createCourseSelector(
...
export const selectViewDetails = createCourseSelector(
...
export const selectSearchDetails = createCourseSelector(
...
export const selectBoardStates = createCourseSelector(
  ...
  (boards, views, router, preferences, course) =>
    mapValues(boards, (board, discussionId) => {
      ...
      const stateParams = router.searchParams; // this is wrong, always undefined
...
export const selectBoardThreads = createCourseSelector(
...
export const selectDiscussionJumpers = createCourseSelector(
...
export const createJumperSelector = (discussionId, jumperKey) =>
  createCourseSelector(
    ...
    (discussionsJumpers, currentUser, boards) => {
      const data: any = get(discussionsJumpers, `[${discussionId}][${jumperKey}]`, {});
