...
const selectUsers = (state: CourseState) => state.api.users;
const selectCompactHeaders = (state: CourseState) => state.ui.progressReport.compactHeaders.status;
...
export const selectProgressReportListState = (state: CourseState) => state.ui.progressReport.listState;

export const selectProgressReportLoaderComponent = (state: CourseState) => ({
  // listReducer is typed loosely (object) in the store, so reach status through a cast.
  loadingState: (state.ui.progressReport.listState as { status: unknown }).status,
});
