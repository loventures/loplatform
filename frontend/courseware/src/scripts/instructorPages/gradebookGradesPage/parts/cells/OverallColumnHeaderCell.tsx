...
interface OverallColumnHeaderCellProps extends WithTranslateProps {
  categoryId?: any;
  syncStatus?: any;
}

const OverallColumnHeaderCell = ({
  translate,
  categoryId,
  syncStatus,
}: OverallColumnHeaderCellProps) => (
...
export default connect((state: any, { categoryId }: { categoryId?: any }) => {
  return {
    categoryId,
    syncStatus:
      categoryId !== '_root_' && latestColumnSyncStatus(state.api.gradebookColumns[categoryId]),
  };
})(withTranslation(OverallColumnHeaderCell));
