...
export const Histories = ({ histories, renderItem }: HistoriesProps) => {
  return histories ? (
...
const Queued = withTranslation(({ time, translate }: any) => (
...
const Attempted = withTranslation((props: any) => {
  ...
  return (
    <span className="history-body">
      ...
      <div className="history-details">
        {detailsVisible ? renderError(props.error, props.translate) : null}
      </div>
    </span>
  );
});
/* eslint-disable angular/json-functions */
const Synced = withTranslation((props: any) => {
  ...
  return (
    <span className="history-body">
      ...
      <div className="history-details">
        ...
        {sourceVisible ? <pre>{JSON.stringify(props.syncedValue, null, 2)}</pre> : null}
      </div>
    </span>
  );
});

const Failed = withTranslation((props: any) => {
  const [detailsVisible, setDetailsVisible] = useState(false);
  return (
    <span className="history-body">
