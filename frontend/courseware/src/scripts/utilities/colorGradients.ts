...
import { gradeFilter } from './gradeFilter.ts';

const gradientClassNames = (percent: number = 0, className?: string): string =>
  classnames(className, `done-${isNumber(percent) ? Math.round(percent) : ''}`);

const gradeToPercent = (grade: any): any => gradeFilter(grade, 'percent', 0);

const progressToPercent = (progress: any): number =>
  progress && isNumber(progress.weightedPercentage) ? progress.weightedPercentage : 0;
export const coloredGrade = (grade: any): string => {
  return gradientClassNames(gradeToPercent(grade), 'colored-grade');
};

export const coloredGradeBg = (grade: any): string => {
  return gradientClassNames(gradeToPercent(grade), 'colored-grade-bg');
};

export const coloredProgress = (progress: any): string => {
  return gradientClassNames(progressToPercent(progress), 'colored-progress');
};

export const coloredProgressBg = (progress: any): string => {
