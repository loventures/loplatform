...
export const getAssetRenderUrl = (
  name: string,
  branch = Course.branch_id,
  commit = Course.commitId
) => {
...
export const getCourseAssetRenderUrl = (path: string) => {
  const hasOffering = !!Course.offering_id;
  const cdn = cdnWebAssets && hasOffering; // only use the CDN in a real section
...
export const getFileBundleUrl = (fileInfo: any, content: any, commit = Course.commitId) => {
...
export const getHotspotImageUrl = (name: string, commit = Course.commitId) => {
  return new UrlBuilder((loConfig.contentItem.asset as any).serve, {
    commit,
    name,
  }).toString();
};

export const getLessonPrintUrl = (
  name: string,
  branch = Course.branch_id,
  commit = Course.commitId
) => {
