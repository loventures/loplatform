...
const srs = {
  ...
  parseSearchString(searchString: string): string[] {
    const terms = reduce(
      searchString.split('"'),
      (list: string[], string: string, index: number) => {
        if (index % 2) {
    ...
    return filter(terms as string[], (s: string) => s.length) as string[];
  },

  fromListOptions(listOptions?: any): any {
    const options = extend({}, srs.defaultOptions, listOptions) as any;
