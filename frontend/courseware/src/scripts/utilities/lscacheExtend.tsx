...
import lscacheModule from 'lscache';
import settings from './settingsService';
import nativeQ from './nativeQ.ts';
import request from './request.ts';

const lscache: any = lscacheModule;
(window as any).lscache = lscache; // expose for debugging purposes
 ...
 *
 * Extends lscache to provide our most common use case. This augments the
 * globally available lscache window object as a top-level side effect of
 * importing this module (formerly an AngularJS factory + run block; now applied
 * directly with the native `$q` shim (nativeQ) and the native `request`
 * singleton). This is a bit hacky, but we would rather augment lscache and just
 * use it rather than include lscacheExtend in every service etc.
 *
 ...
 */
function applyLscacheExtend($q: any, Request: any): any {
          ...
          try {
            if (Request.isValid(data, status)) {
              if (Request.hasResults(data)) {
                ...
                if (isObject(data)) {
                  (data as any).CACHED_AT_TIME = now();
                }
...
// Apply the monkey-patch immediately at import time (top-level side effect),
// replacing the former Angular factory + run block. The native `$q` shim and
// `request` singleton stand in for the injected `$q` / `Request`.
applyLscacheExtend(nativeQ, request);
lscache.isAvailable();

// The only former consumer of the Angular module default export
// (partials/coreDependencies.js, itself dead — no importers) read `.name`;
// index.tsx imports this module purely for its side effect. Export the patched
// lscache so any default-import keeps working without an Angular module.
