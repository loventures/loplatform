...
/**
 * Pure-TS port of the Angular `lo.assignmentGrade.Rubric` module (models/Rubric.js).
 *
 * The grade-card UI re-renders via the GraderProvider's 150ms poll, which reads MUTABLE
 * fields on these objects (`sections[i].dirty / selectedPoints / feedback`, etc.) and the
 * `isDirty()` / `isValid()` surface. This is a lift-and-shift: every mutation and the
 * `onChange` wiring (section → parent Rubric) is preserved verbatim. Do NOT make anything
 * immutable.
 *
 * `Rubric.js` keeps thin Angular adapters (`.service`/`.factory`) over these classes, with
 * the SAME service names (`RubricService`, `Rubric`, `RubricSection`, `ViewRubric`), so the
 * still-Angular consumers that inject them keep resolving.
 */

export const RubricService = {
  defensivelyParseRubric: function (rubric: any) {
    ...
    each(criterion, function (section: any) {
      let sectionPoints = 0;
      each(section.levels, function (level: any) {
...
export const RubricSection = function (this: any, section: any, response: any = {}, onChange?: any) {
  extend(this, section);

  this.onChange = onChange;

  each(this.levels, function (level: any, index: number) {
    level.index = index;
  });
...
RubricSection.prototype.getCriterionResponse = function (forLWC: any) {
...
RubricSection.prototype.setFeedback = function (feedback: any) {
  this.feedback = feedback;
  this.dirty = !isEqual(this.getResponse(), this.initial.response);
};

RubricSection.prototype.reset = function (manual: any) {
  this.set(manual, -1, void 0);
};

RubricSection.prototype.set = function (manual: any, index: any, points: any) {
...
RubricSection.prototype.setManual = function (manualPoints: any) {
...
RubricSection.prototype.setSelection = function (levelIndex: any) {
...
RubricSection.prototype.isValidManualScore = function (selectedPoints: any) {
  return (
    !isNaN(parseFloat(selectedPoints)) && selectedPoints >= 0 && selectedPoints <= this.points
  );
};

RubricSection.prototype.isValidSelection = function (index: any) {
  return !!this.levels[index];
};

export const Rubric = function (this: any, rubric: any, response?: any, onChange?: any) {
  if (isString(rubric)) {
    try {
  ...
  this.sections = map(
    sections,
    function (this: any, section: any, index: number) {
      section.index = index;
      return new (RubricSection as any)(
        section,
        response[index],
        function (this: any) {
          this.onChange(this);
...
Rubric.prototype.getScore = function () {
  return reduce(
    this.sections,
    function (score: any, section: any) {
...
Rubric.prototype.getCompetencies = function () {
  return uniq(
    reduce(
      this.sections,
      (competencies: any, section: any) => [...competencies, ...section.competencies],
      []
    )
  );
};
...
Rubric.prototype.setClean = function () {
  each(this.sections, function (section: any) {
    section.dirty = false;
  });
};
...
Rubric.prototype.getResponse = function () {
  ...
  each(this.sections, function (section: any, index: number) {
...
Rubric.prototype.getCriterionResponse = function (forLWC: any) {
  var response: any = {};
...
Rubric.prototype.isDirty = function () {
  return some(this.sections, function (section: any) {
    return section.dirty;
  });
};

Rubric.prototype.isValid = function () {
  return every(this.sections, function (section: any) {
...
*/
export const ViewRubric = function (this: any, rubricData: any, criterionResponses?: any) {
  const criterion = rubricData.criteria || rubricData.sections;
  this.sections = map(criterion, (criteria: any, index: number) => {
    const criteriaResponse = criterionResponses ? criterionResponses[index] : {};
    return {
      ...criteria,
      title: criteria.title || criteria.name,
      index,
      levels: map(criteria.levels, (level: any, index: number) => ({ ...level, index })),
