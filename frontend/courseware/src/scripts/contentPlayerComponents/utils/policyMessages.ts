...
import { gradeFilter } from '../../utilities/gradeFilter.ts';
import { Translate } from '../../i18n/translationContext';

const createRightsGatingPolicyMessage = (translate: Translate, policy: any, name?: string) =>
  translate('RIGHTS_BASED_LOCK_MESSAGE_' + policy.policyType);

const createTemporalGatingPolicyMessage = (
  translate: Translate,
  policy: any,
  name: string,
  historic: boolean
  ...
  translate(historic ? 'CONTENT_TIME_LOCK_BEFORE_HISTORIC' : 'CONTENT_TIME_LOCK_BEFORE', {
    name,
    time: formatDayjs(policy.lockDate, 'full'),
  });

const createActivityGatingPolicyMessage = (translate: Translate, policy: any, name: string) =>
  translate(
    ...
    {
      name,
      grade: gradeFilter(policy.threshold, 'percentSign'),
      assignments: policy.name,
    }
  );

const isHistoricDate = (lockDate: string | number | Date) =>
  new Date(lockDate).getTime() < new Date().getTime();

//Returns all gating messages in order: Temporal -> Activities -> Rights
const createMessagesForGate = (translate: Translate, gate: any, content: any, viewingAs?: any) => {
  let messages: string[] = [];
...
export const createMessagesForContent = (translate: Translate, content: any, viewingAs?: any) =>
