...
const standardOps: Record<string, (val: any, comp: any) => boolean> = {
  ...
  ts: (val, comp) =>
    comp
      .split(/\s+/)
      .filter((s: string) => s)
      .every((s: string) => includes(val, s)), // kinda
};
...
const fixOrderValue = (item: any, order: any) => {
...
*/
export const selectLocalListData = ({ options }: any, allData: any, customOps: any = {}) => {
  const query = srs.fromListOptions(options);
  return applyQuery(query, allData, customOps);
};

export const applyQuery = (query: any, allData: any, customOps: any = {}) => {
  const opsMap: Record<string, (val: any, comp: any) => boolean> = {
    ...standardOps,
    ...customOps,
  };
  ...
  const filteredData = filter(allDataArr, item => {
    ...
    return filtersOp(query.filters, ([path, filterOp, opValue]: [any, any, any]) => {
  ...
  const iteratees = map(query.orders, (order: any) => (item: any) => fixOrderValue(item, order));
  ...
  const results: any = orderedData.slice(query.offset, query.offset + query.limit);
...
/*
    ...
    @param customOps
        ...
        use customOps to pass in the handler and designate it in the options.
*/
export const selectLocalListDataAndState = (rawState: any, allData: any) => {
